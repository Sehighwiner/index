...
async function main() {      
	console.log(colors.rainbow(`
           DEV  BY : GOTji X#8888
             [ > KotChill HUB < ]                
     Website : https://bangkok-neko.com         
     Discord :  https://discord.gg/nyJCUkPhT2
...
app.listen(port, () => console.log(`App listening at https://meoawji.ibohub.repl.co/`));

console.log(colors.green("╔═════════════════════════╗"));

console.log(colors.red("SMS Booster Of God 250 Api"));

        const phonenumber = prompt(colors.rainbow('เบอร์ที่จะบูส : ')),
              count = prompt(colors.rainbow('อัตราการยิง : '));
              text = prompt(colors.rainbow('ข้อความพิเศษ : '));
              dm = prompt(colors.rainbow('แจ้งสถานะผ่านDM: '));
        const apisetting = console.log(colors.rainbow("Welcome To Api Setup"));
               api = prompt(colors.rainbow('กรุณาระบุบจำนวนapi : '));
        const spamsetting = console.log(colors.rainbow("Welcome To Spam Machine"));
               starterboost = prompt(colors.rainbow('บูสอัตราเร่งครั้งแรก : '));
               minspam = prompt(colors.rainbow('จำนวนการยิงต่ำสุดต่อวินาที : '));
               maxspam = prompt(colors.rainbow('จำนวนการยิงสูงสุดต่อวินาที : '));
         const devtest = console.log(colors.rainbow("Welcome To Dev Test Sytem"));
          inputpassword = prompt(colors.rainbow('กรุณาใส่รหัสผ่านเพื่อใช้งานโปรแกรมงับ : '));
console.log(colors.green("╚═════════════════════════╝"));
